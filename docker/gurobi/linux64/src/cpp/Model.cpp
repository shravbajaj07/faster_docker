...
GRBModel::optimize()
{
  ...
  if (cb != NULL)
    cb->setcb(cb, Cmodel, cols, wheres);
...
{
  int error = GRBresetparams(GRBgetenv(Cmodel));
  if (error) throw GRBException(string(GRBgeterrormsg(Cenv)), error);
}
              ...
              int                  len)
{
  ...
  if (xgenconstrs == NULL) throw
    GRBException("Invalid arguments", GRB_ERROR_INVALID_ARGUMENT);
  checkattrsize(Cmodel, dattrname[attr], 5);
  double* val = new double[len];
  int* ind = setgenconstrsind(xgenconstrs, len);
  int error = GRBgetdblattrlist(Cmodel, dattrname[attr], len, ind, val);
  delete[] ind;
              ...
              int                  len)
{
  ...
  checkattrsize(Cmodel, iattrname[attr], 5);
  int* ind = setgenconstrsind(xgenconstrs, len);
  int error = GRBsetintattrlist(Cmodel, iattrname[attr], len, ind, (int*) val);
  delete[] ind;
  if (error) throw GRBException(string(GRBgeterrormsg(Cenv)), error);
}
              ...
              int                  len)
{
  ...
  checkattrsize(Cmodel, dattrname[attr], 5);
  int* ind = setgenconstrsind(xgenconstrs, len);
  int error = GRBsetdblattrlist(Cmodel, dattrname[attr], len, ind, (double*) val);
  delete[] ind;
  if (error) throw GRBException(string(GRBgeterrormsg(Cenv)), error);
}
...
void
GRBModel::setCallback(GRBCallback* xcb, unsigned int wheres)
{
