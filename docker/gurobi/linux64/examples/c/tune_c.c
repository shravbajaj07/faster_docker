     ...
     char *argv[])
{
  ...
  /* Retrieve model env to set some parameters */

  modelenv = GRBgetenv(model);
  assert(modelenv != NULL);

  /* Set a time limit for the model */

  error = GRBsetdblparam(modelenv, GRB_DBL_PAR_TIMELIMIT, 5);
  if (error) goto QUIT;
   ...
   * tuning tool can test several hundreds of parameter settings.
   */

  error = GRBsetdblparam(modelenv, GRB_DBL_PAR_TUNETIMELIMIT, 60);
  if (error) goto QUIT;
   ...
   * setting.
   */
  error = GRBsetintparam(modelenv, GRB_INT_PAR_TUNERESULTS, 2);
  if (error) goto QUIT;
